...
Jako zajímavost jsme se dozvěděly, že o `is` a `==` se točí v pythonním světě diskuze.
...
Na pomoc nám přijde _context manager_.
...
Můžeme je vytvořit několika způsoby. První je vytvořit třídu, která má dvě metody:
```pycon
>>> class Kontext:
...     def __enter__(self):
...         print("začátek")
...
Další věc, kterou umí dělat context manager, je to, že dokáže naši chybu chytit a předat ji nám na další zpracování.
```pycon
>>> class Kontext:
...     def __enter__(self):
...         print("začátek")
...
Můžeme si napsat jednodušší context manager, který udělá totéž jako naše třída výše.
